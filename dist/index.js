...
function maybePromised(_this, callback, wrapped) {
  if ('function' === typeof callback) {
    ...
    wrapped(callback); // Return `this` to keep the same behaviour Cacheman had before promises were added
    ...
    value: function run(key, data, ttl, fn) {
      ...
      var go = function go(i) {
        fns[i](key, data, ttl, function (err, _data, _ttl, _force) {
          ...
          if (!fns[i + 1]) return fn(null, _data, _ttl, _force); // go on to next
